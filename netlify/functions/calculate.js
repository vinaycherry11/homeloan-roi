...
function runSimulation(homePrice, monthlyRent, interestRate, appreciation, rentAppreciation, maintenancePct, stockReturn, tenure, downPct) {
  ...
  let snpEmiSip = down;     // down payment lump sum + EMI monthly SIP at S&P rate
  let surplusSnp = 0;
  ...
  const surplusGrowth = surplusSnp - surplusInvested;   // S&P compounding gain on reinvested surplus
  const finalBenefit = finalEquity + cumRent + surplusGrowth;

  const homeNetGain = finalEquity + cumNetCf;   // equity + (total rent - total EMI - total maintenance)
  ...
  const stockNetGain = finalSnp;                // down payment + monthly outflow invested in S&P 500
  const homeVsSnp = homeNetGain - stockNetGain; // positive = home wins vs S&P 500 renter
  const homeWealthFinal = finalBenefit;               // equity + cumRent + surplusGrowth
...
exports.handler = async function (event) {
  ...
  if (!home_price || home_price < 100000) {
    return { statusCode: 400, body: JSON.stringify({ detail: 'Home price must be at least ₹1 lakh' }) };
  }
